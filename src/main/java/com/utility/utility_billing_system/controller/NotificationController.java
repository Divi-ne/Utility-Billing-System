...
public class NotificationController {
    ...
    /** Lists all notifications for the logged-in user (no user ID required). */
    @GetMapping("/me")
    @PreAuthorize("hasAnyRole('ADMIN', 'CUSTOMER', 'OPERATOR', 'FINANCE')")
    @Operation(summary = "Get my notifications")
    public ResponseEntity<ApiResponse<List<NotificationResponse>>> getMyNotifications() {
        return ResponseEntity.ok(wrap(notificationService.getMyNotifications(), "Notifications retrieved"));
    }

    /** Lists unread notifications for the logged-in user (no user ID required). */
    ...
    public ResponseEntity<ApiResponse<List<NotificationResponse>>> getMyUnreadNotifications() {
        return ResponseEntity.ok(wrap(notificationService.getMyUnreadNotifications(), "Unread notifications retrieved"));
    }

    /** Lists all notifications for a user, newest first. Role: ADMIN. */
    ...
    /** Lists only unread notifications for a user. Role: ADMIN. */
