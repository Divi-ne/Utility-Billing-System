...
public class TariffService {
    ...
    @Transactional
    /** Updates an existing tariff's rates and tier definitions. */
    public TariffResponse updateTariff(Long id, TariffUpdateRequest request) {
        ...
        tariff.setLatePaymentPenaltyPercentage(request.getLatePaymentPenaltyPercentage());
        ...
        validateTariffHasPricing(tariff);

        log.info("Tariff {} updated for {}", tariff.getId(), tariff.getMeterType());
    ...
    private void validateTariffHasPricing(Tariff tariff) {
        boolean hasFlatRate = tariff.getFlatRate() != null;
        boolean hasTiers = tariff.getTiers() != null && !tariff.getTiers().isEmpty();
        if (!hasFlatRate && !hasTiers) {
            throw new BusinessRuleException("Tariff must have either a flat rate or tier-based rates");
        }
    }
