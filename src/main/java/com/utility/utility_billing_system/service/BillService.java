...
public class BillService {
    ...
    public BillResponse generateBill(BillGenerateRequest request) {
        ...
        log.info("Bill generated for meter {} - {}/{}, total: {}",
                meter.getMeterNumber(), request.getBillingMonth(), request.getBillingYear(), totalAmount);

        if (meter.getCustomer().getUser() != null) {
            notificationService.createNotification(
                    meter.getCustomer().getUser(),
                    "New Bill Generated",
                    String.format("Your bill for %d/%d is ready. Amount: %s",
                            request.getBillingMonth(), request.getBillingYear(), totalAmount),
